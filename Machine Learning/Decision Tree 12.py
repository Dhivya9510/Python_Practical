...
df = pd.read_csv("resources/Decision-Tree.csv")
...
X = df[features]          # Note NO double quotes here in 'Featured column'.
y = df["Go"]
...
df = pandas.read_csv("resources/Decision-Tree.csv")
...
print(dtree.predict([[40,10,7,1]]))     # Note the format. 
...
# Own notes: 
   # Step by step:
        # Import modules
        # Input variable with array for access. 
        # Create map
        # Feature column & target column input. 
        # Decision tree line items
        # plot it. 
        # Understood line items 67 to 70 (the input for Decision tree map)
        # Note the line item 162 & 167 (the bracket format)
